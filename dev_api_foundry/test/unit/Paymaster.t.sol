...
contract PaymasterTest is Test {
    ...
    address internal constant UNLISTED_VOTER = 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC;
    uint256 internal constant UNLISTED_VOTER_PKEY = 0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d;
    ...
    function test_validatePaymasterUserOp_ValidProof() public {
        ...
        PackedUserOperation memory userOp = _buildUserOp(VOTER_1, abi.encode(proof));
        bytes32 userOpHash = entryPoint.getUserOpHash(userOp);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(VOTER_1_PKEY, userOpHash);
        userOp.signature = abi.encodePacked(r, s, v);

        vm.prank(address(entryPoint));
        (, uint256 validationData) = paymaster.validatePaymasterUserOp(userOp, userOpHash, 1e18);

        assertEq(validationData, SIG_VALIDATION_SUCCESS, "Validation should succeed");
    }

    function test_validatePaymasterUserOp_InvalidProof() public {
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(VOTER_1)); // Wrong proof element
        ...
        assertEq(validationData, SIG_VALIDATION_FAILED, "Validation should fail for invalid proof");
    }
    ...
    function test_validatePaymasterUserOp_UnlistedVoter() public {
        ...
        PackedUserOperation memory userOp = _buildUserOp(UNLISTED_VOTER, abi.encode(proof));
        bytes32 userOpHash = entryPoint.getUserOpHash(userOp);
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(UNLISTED_VOTER_PKEY, userOpHash);
        userOp.signature = abi.encodePacked(r, s, v);

        vm.prank(address(entryPoint));
        (, uint256 validationData) = paymaster.validatePaymasterUserOp(userOp, userOpHash, 1e18);

        assertEq(validationData, SIG_VALIDATION_FAILED, "Validation should fail for unlisted voter");
    }
